...
function ProductList() {
  ...
  const [typeInCart, setTypeInCart] = useState(0);
  ...
  useEffect(() => {
    const categoryCount = cart.reduce((total, item) => total + item.quantity, 0);
    setTypeInCart(categoryCount); 
}, [cart]);

useEffect(() => {
  const checkDisabled = cart.map(item => item.name);
  setDisabledProducts(checkDisabled)
}, [cart]);
          ...
          <div>
            ...
            <a href="#" onClick={(e) => handleCartClick(e)} style={styleA}>
              <h1 className="cart" style={{ position:"relative"}}>
                ...
                <p style={{ position: "absolute", top: "50%", left: "52%", transform: "translate(-50%, -50%)", fontSize:"28px" }}>{typeInCart}</p>
              </h1>
            </a>
          </div>
        </div>
              ...
              <div className="product-list">
                {item.plants.map((plant, plantIndex) => (
                  <div className="product-card" key={plantIndex}>
                    ...
                    <button
                    className={`product-button ${disabledProducts.includes(plant.name) ? "disabled" : ""} `}
                    onClick={() => handleAddToCart(plant)}
                    disabled={disabledProducts.includes(plant.name)}>
                    {disabledProducts.includes(plant.name) ? "Added to Cart" : "Add to Cart"}</button>
                  </div>
                ))}
              </div>
            </div>
